...
function displayWinner(candidates) {
  const getWinner = (candidates) => {
    // Returns an array containing the winner(s) based on maximum vote count
    ...
    // Find candidate(s) with maximum vote count
    for (let i = 0; i < candidates.length; i++) {
      if (candidates[i].voteCount === maxVoteReceived) {
  ...
  const renderWinner = (winner, index) => {
    return (
      <div key={index} className="container-winner">
        <div className="winner-info">
          <p className="winner-tag">Winner!</p>
          <h2>{winner.header}</h2>
          <p className="winner-slogan">{winner.slogan}</p>
  ...
  return (
    <>
            {winners.map((winner, index) => renderWinner(winner, index))}
